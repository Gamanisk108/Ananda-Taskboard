...
(function () {
  ...
  const attach = (file) => file
    ...
    : `<button class="an-attach"><span class="an-att-ic">${I.paperclip}</span><span class="an-att-tx">Attach a screenshot <span class="opt">(optional)</span></span></button>`;
  ...
  function webReport(state) {
    if (state === 'sent') return wrapModal(flowModal('Report a problem', null,
      success({ title: 'Report received', body: 'Thank you — we’ve logged this and the team will take a look. We may follow up by email if we need more detail.', ref: 'TR-4821' }), doneFoot));
  ...
  function featureBody(state) {
    ...
    return `<div class="flow-intro">${I.sparkle}<p>Have an idea that would make the board better for your community? We read every one.</p></div>
      ...
      <div class="an-techbox">${toggle('Tell me when this ships', true)}</div>`;
  }
  ...
  function spreadBody(state) {
    ...
    return `<div class="flow-intro">${I.share}<p>Invite another Ananda center or seva team to keep their work here too.</p></div>
      <div class="sec-label">Invite by email</div>
      <div class="field"><label>Their email</label><input type="email" placeholder="name@center.org" /></div>
      <div class="field"><label>Add a note <span class="lbl-opt">(optional)</span></label>
        <textarea class="fl-area" rows="2" placeholder="Joy! We use this to run our seva — thought your team might like it too."></textarea></div>
      <button class="btn btn-primary fl-inline-send">${I.send} Send invitation</button>
      <div class="an-or"><span>or share a link</span></div>
      ${copyrow('https://taskboard.ananda.org/join/karuna-devi', copied)}
      ${shareRow()}
      <div class="note an-spread-note">${I.info} Anyone with the link can request to join — an admin approves them.</div>`;
  }
  function webSpread(state) {
    if (state === 'sent') return wrapModal(flowModal('Spread the word', null,
      success({ title: 'Invitation sent', body: 'We’ve emailed an invite to <b>seva@anandaseattle.org</b>. They’ll get a link to join your team.', quote: ['Where people pull together in a common ideal, miracles happen.', 'Swami Kriyananda'] }), doneFoot));
    return wrapModal(flowModal('Spread the word', 'Invite another center or seva team',
  ...
  function phoneReport(state) {
    if (state === 'sent') return mScreen('Report a problem', mSuccess({ title: 'Report received', body: 'Thank you — we’ve logged this and the team will take a look.', ref: 'TR-4821' }), mDoneFoot);
  ...
  function phoneSpread(state) {
    if (state === 'sent') return mScreen('Spread the word', mSuccess({ title: 'Invitation sent', body: 'We’ve emailed an invite to <b>seva@anandaseattle.org</b>.', quote: ['Where people pull together in a common ideal, miracles happen.', 'Swami Kriyananda'] }), mDoneFoot);
