...
## RULINGS 2026-06-11 (from `uploads/QUESTIONS-FOR-DESIGN-2026-06-11.md` · mocked in `rulings-06-11/Ananda Taskboard - Rulings 06-11.html`)

### D49 · Full-screen task view (phones) — sticky footer blessed; Share becomes a header icon · 2026-06-11
- **Ruled (Gordon):** the interim build's **sticky footer is the standard full-screen treatment**,
  with one change — **Share leaves the footer and becomes a ghost share ICON in the `.fs-head`**,
  at the right end (where the old mock's Save sat). Echoes D12's header Share on the subtask
  breadcrumb.
- **Spec (edit view):** fs-head = back · title (+pen) · #id chip · **share icon** (42px ghost,
  `--r-ctl` radius, muted stroke). Footer pinned under the scroll body, `border-top` 1px border:
  **Delete (left, red outline per D5) · spacer · Cancel · Save (filled primary)**. The mock's
  body-bottom full-width Delete is **retired** — Delete lives only in the footer.
- **Spec (create view):** same shared footer — **spacer · Cancel · “Create task”** (label per
  D27); no Delete, no Share. The mock's body-end Create button is retired.
- **Back chevron stays pure navigation**; Cancel lives in the footer. Both trigger the
  unsaved-changes confirm when dirty (app confirm rule).
- Surfaces: **MOB** — `mobile/screens-flows.js` `taskDetail()`/`newTask()` are now **stale**
  (final markup in `rulings-06-11/`); **BUILD** — keep the footer, reorder to Delete · Cancel ·
  Save, move Share up into the fs-head as the icon. **APP (web dialogs)** unchanged — web keeps
  the D5 footer (Delete · Share · Cancel · Save); this ruling is phones-full-screen only.

### D50 · APR-4 — member visibility of pending-approval tasks = in-place pill + account-menu entry (A + C) · 2026-06-11
- **Ruled (Gordon):** combination of **A (in place)** and **C (account-menu notice)**. A pending
  task never vanishes from the member's board.
- **Spec A · board (web List):** the row stays **in place** (normal sort), **read-only**, with a
  warm-**gold row tint** (`color-mix(in srgb, var(--gold) 9%, transparent)` — same vocabulary as
  the red/amber tints) and a **“Pending approval” pill in the Status column** — warn-gold, **no
  caret** (it is not a status, not interactive). A pending **EDIT** keeps showing the last
  **approved** values + a small **“Edit pending”** pill after the task name. The **summary strip**
  gains a gold **“N Pending approval”** chip — hidden at 0; pending items are NOT counted into
  the task totals. The transient submit toast stays (“Sent for approval — an admin will
  review”). Phone compact row: **gold status rail** (`--sc: var(--gold)`) + tint; the pill takes
  the **status pill's slot in the D51 right column** (date stays on line 2); pending EDIT keeps
  its live status pill with the small “Edit pending” pill **stacked beneath it** in the right
  column. The task-detail “Pending approval” pill (already in the
  mobile mock) stays — detail opens read-only.
- **Spec C · menu entry:** web account menu — **“Pending approval” + mono count badge** as the
  first item under the menu header; phone — in the **⋯ overflow where Approvals sits for
  admins**. **Hidden at N=0** (same rule as Unscheduled, D6).
- **Spec C · destination (proposed 06-11, confirm):** opens **“Waiting for approval”** — web: a
  dialog (max-width 760) on the Approvals-list methodology: **sort** (Newest first) + **kind**
  (All kinds / New tasks / Edits) custom selects (no full filter bar — personal volume); table
  **Task · Project · Sub-project · Requested · Submitted**; Requested = **NEW task** chip (azure)
  or **EDIT · n changes** chip (gold) — edits expand an **old → new diff row per field** (mono,
  sunk bg). Rows open the read-only task popup carrying the same pill. Phone: a **full-screen
  route** reusing the Approvals `.crow` list minus approve/reject (chevron instead), NEW/EDIT
  chips, “sent {date}”, diff line on edits.
- **Pill copy:** sentence-case **“Pending approval”** (matches the agreed detail pill + app copy
  style). Gordon wrote “Pending Approval” — **confirm casing**.
- **Assumptions to confirm:** member topbar drops **Approvals** only (Team + Projects kept);
  member account menu strips History / Archive / Restore points / Trash / Preview-as-Viewer.
- Surfaces: **APP / MOB / BUILD** (this is the APR-4 fix — Code action) · mocks in
  `rulings-06-11/` · i18n: new strings (pill ×2, menu item, dialog title/sub/columns/chips,
  toast) → all 13 locales.
- **Amended same day (Gordon):** on phones the pending pill moves to the **right side** — it
  takes the status pill's slot in the D51 right column (the date returns to line 2); and the
  **"Waiting for approval" route rows are the List compact row, verbatim** ("match the List
  design exactly"): full width edge-to-edge, **gold band left**, project + **sub-project
  pills**, "sent {date}" in the date slot, right column = **NEW/EDIT chip with the assignee(s)
  beneath**, no chevron; edits keep the full-width old → new diff line. The web dialog matches
  the List methodology too — an **Assignees column** added (Task · Project · Sub-project ·
  Assignees · Requested · Submitted).

### D51 · Responsive List — status pills restored to compact rows (regression fix) · 2026-06-11
- **Issue (Gordon):** the phone/responsive List rows showed **no visible status** — only the
  colored left rail. The canonical responsive card (`.tcard`, faithfully ported across modules)
  has **always carried the status pill** in `tc-meta`; the "Compact rows (selected)" direction
  (`compactRow`/`.trow` in `mobile/screens-views.js`) dropped it. Ruled a regression — restore.
- **Spec:** every compact row gains a **right-aligned column**: the **status pill** (10.5px,
  dot + name, `--sc` tint — the house pill) on top, the **assignee avatars beneath** it. The
  status rail stays (color cue at a glance; the pill names it). The **chevron is dropped** to
  pay for the width — rows are tappable end-to-end ("fill width", per Gordon). Pending rows
  (D50) show the **"Pending approval" pill in that same right slot** — a pending NEW task has
  no live status yet; pending EDIT rows keep their live status pill + the small "Edit pending"
  pill stacked beneath it in the right column (line 2 must stay one line at 390px).
- Day-divider groups, overdue/due-soon tints + badges, priority chevrons — unchanged.
- Surfaces: **MOB** `compactRow()`/`listCompact()` (now stale — final markup in
  `rulings-06-11/rulings-screens.js` `enhanceRows`) · **BUILD** wherever compact List rows
  render · search results (`searchScreen`) and any other `compactRow` consumers inherit it.
