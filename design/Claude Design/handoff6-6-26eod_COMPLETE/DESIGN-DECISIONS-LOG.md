...
>
> _Synced from Claude Design 2026-06-09 (Mythos custodianship). ⚠ Numbering note: Design's
> 2026-06-09 session reused **D36** for the Settings section-nav, displacing the original
> 2026-06-08 "no hard-clipping" entry — restored below as **D42**. Cite D42 (not D36) for the
> clipping rule from now on._
...
> ⚠ Numbering note: this D36 (Design, 2026-06-09) displaced the earlier 2026-06-08 D36
> "no hard-clipping" entry — that rule is preserved below as **D42**.
...
- **Voice/type:** TWO treatments. **(a) Lite** (working surfaces like the Help Us hub): no box, no
  ...
  never compete. **(b) Boxed** (`TRB.quote`): gold-tinted card w/ lotus mark — reserved for
...
## Restored entry — numbering-collision fix (2026-06-09, Mythos custodianship)

### D42 · No hard-clipping of text — graceful overflow everywhere (philosophy)
> Formerly numbered **D36** (2026-06-08); Design's 2026-06-09 session reused D36 for the
> Settings section-nav, dropping this entry. Restored verbatim here. Any reference to "D36"
> for text clipping (older notes, handoff CLAUDE.md) now means **D42**.
- **Principle:** text is **never** sliced mid-character or run under an adjacent
  element. Overflow degrades gracefully, **context-appropriate**:
  - **Single-line label/value** → `text-overflow:ellipsis` (…) with the full text
    still reachable: a `title`/tooltip on hover (or focus-to-reveal for inputs).
  - **Editable input** (e.g. the task-title header) → ellipsis when blurred. Do
    **not** autofocus an existing long value — that pins the cursor to the start
    and shows a clipped beginning with no "…"; only autofocus when **creating**.
    Add a `title` so the full value is available on hover.
  - **Lists / avatar stacks** → a **"+N"** overflow affordance.
  - **Long prose** → wrap.
- **Spec:** flex/grid children that hold text get `min-width:0` (grids
  `minmax(0,1fr)`) so they can shrink and ellipsize instead of forcing overflow.
  The task-title input: `text-overflow:ellipsis` + `autoFocus={!editing}` +
  `title={title}`.
- **Why:** the owner flagged a clipped long task name; clipping is never
  acceptable — it hides information with no affordance to recover it.
- Surfaces: **APP** (task title ✅; audit list cells, proj-pills, chips,
  breadcrumbs, calendar event names, picker rows). **MOB/MT/AUTH/HELP** — same
  rule; line-art truncation, never a hard cut. Mirrored into `CLAUDE.md` Hard UI
  rules.
