...
function renderWeekly(){
  ...
  list.filter(t=>t.due||t.start).forEach(t=>{
    ...
    bars+=`<div class="wk-bar ${ring} ${done?'donebar':''}" style="grid-column:${cs+1}/${ce+2};grid-row:${r};background:${col};position:relative">
      <span class="tt">${wico}<span class="tn">${esc(t.name)}</span></span>${av?`<span class="mini">${av.group?'◇':av.init}</span>`:''}${(cs<todayIdx&&todayIdx<=ce)?`<span class="tn-today" style="position:absolute;top:0;bottom:0;left:${(todayIdx-cs)/(ce-cs+1)*100}%;width:${100/(ce-cs+1)}%;display:flex;align-items:center;justify-content:space-between;gap:6px;padding:0 9px;white-space:nowrap;font-weight:600;pointer-events:none"><span style="overflow:hidden;text-overflow:ellipsis;display:flex;align-items:center;gap:5px;pointer-events:auto">${wico}${esc(t.name)}</span>${av?`<span class="mini" style="flex:none;background:${av.color}">${av.group?'◇':av.init}</span>`:''}</span>`:''}</div>`;
  });
...
function openTeamAdmin(){
  openSheet("Team &amp; Permissions","<div class='seg' id='teamSeg' style='margin-bottom:10px'><button class='on' data-tab='members'>Members</button><button data-tab='groups'>Groups</button><button data-tab='access'>Access</button><button data-tab='tiers'>Tiers</button></div><div id='teamDesc' class='note' style='margin-top:0'></div><div id='teamBody'></div>");
  const DESCS={members:"People on the board and the tier each one inherits their default access and visibility from.",groups:"Reusable groups of people — assign a whole group to a task at once.",access:"Grant a person, group, or tier access to a project or sub-project, as a member (can edit) or viewer.",tiers:"Reusable permission templates; a new member assigned a tier inherits its grants and visibility."};
  const render=tab=>{
...
function wireAccess(){
  ...
  const who={Person:["Ada","Mara","Lila Devi","Lakshmi","Arjuna"],Group:["Seva","Kitchen"],Tier:["Volunteer","Coordinator","Lead"]};
...
const TEAM_TABS={
  members:`<div class="card" style="padding:12px;margin-bottom:14px;background:var(--sunk)">
    ...
    <div class="field" style="margin-bottom:0"><label>Tier <span class="muted" style="font-weight:400">(new member inherits this tier's access + visibility)</span></label><select style="max-width:380px"><option>Volunteer — sees own tasks only</option><option>Coordinator — all tasks in the sub-project</option><option>Lead — all tasks across the project</option></select></div>
    <button class="btn-primary" style="margin-top:12px" data-toast="Member added">Add member</button>
    <div class="muted" style="font-size:12px;margin-top:6px">Share the email + password with them directly. Admins ignore tiers — they see everything.</div></div>
  <table class="tbl"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Tier</th><th>Status</th><th></th></tr></thead><tbody>
    <tr><td>Ada</td><td class="muted">admin@ananda.test</td><td><select><option>Admin</option><option>Member</option></select></td><td><span class="pill" style="opacity:.75">Administrator</span></td><td><select><option>Active</option><option>Suspended</option></select></td><td style="white-space:nowrap"><button class="btn-ghost" data-toast="Password reset link sent">Reset pw</button><button class="btn-ghost rmMember icon-btn" data-name="Ada" title="Remove" style="color:var(--danger)">${ICN_X}</button></td></tr>
    <tr><td>Mara</td><td class="muted">mara@ananda.test</td><td><select><option>Member</option><option>Admin</option></select></td><td><select><option>Coordinator</option><option>Volunteer</option><option>Lead</option></select></td><td><select><option>Suspended</option><option>Active</option></select></td><td style="white-space:nowrap"><button class="btn-ghost" data-toast="Password reset link sent">Reset pw</button><button class="btn-ghost rmMember icon-btn" data-name="Mara" title="Remove" style="color:var(--danger)">${ICN_X}</button></td></tr>
    <tr><td>Lila Devi</td><td class="muted">lila@ananda.test</td><td><select><option>Member</option><option>Admin</option></select></td><td><select><option>Volunteer</option><option>Coordinator</option><option>Lead</option></select></td><td><select><option>Active</option><option>Suspended</option></select></td><td style="white-space:nowrap"><button class="btn-ghost" data-toast="Password reset link sent">Reset pw</button><button class="btn-ghost rmMember icon-btn" data-name="Lila Devi" title="Remove" style="color:var(--danger)">${ICN_X}</button></td></tr>
  </tbody></table>`,
  ...
  access:`<div class="card" style="padding:12px;margin-bottom:14px;background:var(--sunk)"><h3 class="section-title">Grant access</h3>
    <div class="row2"><div class="field"><label>Give access to</label><div style="display:flex;gap:6px"><select id="accWho" style="width:120px"><option>Person</option><option>Group</option><option>Tier</option></select><select id="accWhoSel"></select></div></div>
  ...
  tiers:`<div class="field" style="display:flex;gap:8px"><input placeholder="New tier name… (e.g. Volunteer)" /><button class="btn-primary" data-toast="Tier created">Add tier</button></div>
  <div class="card" style="padding:12px;margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong>Volunteer</strong><span style="display:flex;gap:6px;align-items:center"><span class="pill">3 members</span><button class="btn-secondary" data-toast="Edit tier access">Edit access →</button><button class="btn-danger" data-toast="Tier deleted">Delete</button></span></div><div class="muted" style="font-size:12px">Default visibility <strong>Own tasks only</strong> · grants: Karuna Devi / Marketing (member), Sunday Service / Seva (viewer)</div></div>
  <div class="card" style="padding:12px;margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong>Coordinator</strong><span style="display:flex;gap:6px;align-items:center"><span class="pill">2 members</span><button class="btn-secondary" data-toast="Edit tier access">Edit access →</button><button class="btn-danger" data-toast="Tier deleted">Delete</button></span></div><div class="muted" style="font-size:12px">Default visibility <strong>All tasks in the sub-project</strong> · grants: Karuna Devi (whole project, member)</div></div>
  <div class="card" style="padding:12px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong>Lead</strong><span style="display:flex;gap:6px;align-items:center"><span class="pill">1 member</span><button class="btn-secondary" data-toast="Edit tier access">Edit access →</button><button class="btn-danger" data-toast="Tier deleted">Delete</button></span></div><div class="muted" style="font-size:12px">Default visibility <strong>All tasks in the project</strong> · grants: all projects (member)</div></div>`
};
