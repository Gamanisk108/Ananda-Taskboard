...
data is seeded, plus one task pre-trashed by Mara (see the shell setup step).

Run via: with_server.py --server "...runserver 8003" --port 8003 -- python qa/reduced_access_smoke.py
"""
...
with sync_playwright() as p:
    ...
    # Topbar: Trash now reachable by a member; admin-only surfaces still hidden.
    ...
    # User menu: Bulk + Daily reminder available; admin App-Settings still hidden.
    ...
    # Open Bulk: status + deadline only, no move/reassign/archive.
    ...
    # Trash: member sees ONLY their own deleted task, and can restore it.
    ...
    if page.locator('button:has-text("Restore")').count() > 0:
        ...
        # Restore is async (POST + refetch). The row leaves the TRASH MODAL (it
        # also reappears in the main list behind it, since it's live again — so
        # scope the "gone" check to the modal, not the whole page).
